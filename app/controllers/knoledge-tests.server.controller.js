 ...
 */
exports.list = function(req, res) { KnoledgeTest.find().sort('-created').populate('user', 'displayName').exec(function(err, knoledgeTests) {
		if (err) {
			return res.send(400, {
				message: getErrorMessage(err)
			});
 ...
 */
exports.knoledgeTestByID = function(req, res, next, id) { KnoledgeTest.findById(id).populate('user', 'displayName').exec(function(err, knoledgeTest) {
